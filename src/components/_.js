...
//var AMPCOLOUR="#408040";
//var FXCOLOUR="#CCCC00";
var BACKCOLOUR = "#fafafa";
var GRIDCOLOUR = "#DDDDDD";
var AMPCOLOUR = "#2a6cad";
var FXCOLOUR = "#b8dcff";
...
function doRun() {
  ...
  if (micsource == null) {
    ...
    //,startrecording, function() { alert('getUserMedia() failed - use https: address'); });
  }
  ...
  if (recording) {
    recording = 0;
    paused = 0;
    document.getElementById("runbutton").firstElementChild.src =
      "https://asrgropalign.eppmex2022.com/images/microphone_wav.png";
    window.cancelAnimationFrame(anireq);
    closedown();
  } else {
    recording = 1;
    paused = 0;
    document.getElementById("runbutton").firstElementChild.src =
      "https://asrgropalign.eppmex2022.com/images/stop.png";
    signal = new Array();
    track = new Array();
    anallen = SRATE / 20;
    analpos = 0;
      ...
      if (i >= 0 && track[i][0] > 0) {
        ...
        if (track[i + 1][0] > 0 && change < 0.2) {
          for (x = x1; x <= x2; x++) {
            ...
            ctx.moveTo(
              x,
              ctx.height - ((1 - m) * y1 + m * y2) + ((1 - m) * w1 + m * w2) / 2
            );
            ctx.lineTo(
              x,
              ctx.height - ((1 - m) * y1 + m * y2) - ((1 - m) * w1 + m * w2) / 2
  ...
  for (var i = offset; i < track.length - 1; i++) {
    if (i >= 0 && track[i][0] > 0) {
      ...
      var y2 =
        ...
        (logfxmax - logfxmin);
      var change = Math.abs(
        (2 * (track[i][0] - track[i + 1][0])) / (track[i][0] + track[i + 1][0])
      );
      if (track[i + 1][0] > 0 && change < 0.2) {
